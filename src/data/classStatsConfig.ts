...
export type ScaleStat = 'atk' | 'def' | 'maxHp' | 'speed' | 'utility';
...
export interface SkillRatioDef {
  ...
  label: string;
  /** Ratios affichés dans le grimoire (ex. 180 % ATK, 75 % DEF) */
...
const DAMAGE_SCALE_STATS: ScaleStat[] = ['atk', 'def', 'maxHp'];

export interface ClassBaseStatsDef {
  maxHp: number;
  atk: number;
  /** Guerrier : valeur de base ajoutée à la DEF pour les dégâts */
  def?: number;
  speed: number;
...
export const CLASS_STATS_CONFIG: Record<ClassId, ClassStatsDef> = {
  warrior: {
    classId: 'warrior',
    base: { maxHp: 250, atk: 32, def: 32, speed: 13, attackMaxCooldown: 0.9, cooldowns: { space: 5, shift: 6, e: 7 } },
    skills: {
      primary: { ratio: 1.2, label: 'Attaque', scaling: [{ stat: 'def', mult: 1.2 }] },
      space: { ratio: 3.0, label: 'Frappe Sismique', scaling: [{ stat: 'def', mult: 3.0 }] },
      shift: { ratio: 1.5, label: 'Cri de Guerre', scaling: [{ stat: 'def', mult: 1.5, suffix: ' soin' }] },
      e: { ratio: 0, label: 'Parade', scaling: [{ stat: 'def', mult: 0.75, suffix: ' réduction' }] },
    },
    branchSkills: { rempart: null, fureur: 'primary', cri: 'shift', seisme: 'space' },
  },
  mage: {
    classId: 'mage',
    base: { maxHp: 100, atk: 45, speed: 15.5, attackMaxCooldown: 0.6, cooldowns: { space: 5, shift: 8, e: 12 } },
    skills: {
  ...
  sentinel: {
    classId: 'sentinel',
    base: { maxHp: 140, atk: 32, speed: 14.5, attackMaxCooldown: 0.7, cooldowns: { space: 5, shift: 12, e: 12 } },
    skills: {
  ...
  blade: {
    classId: 'blade',
    base: { maxHp: 200, atk: 31, speed: 17, attackMaxCooldown: 0.35, cooldowns: { space: 4, shift: 8, e: 8 } },
    skills: {
      primary: {
        ratio: 0,
        label: 'Attaque',
  ...
  pacifier: {
    classId: 'pacifier',
    base: { maxHp: 190, atk: 38, speed: 15, attackMaxCooldown: 0.5, cooldowns: { space: 5, shift: 8, e: 12 } },
    skills: {
  ...
  chronoregulator: {
    ...
    base: { maxHp: 155, atk: 42, speed: 15.5, attackMaxCooldown: 1.15, cooldowns: { space: 6, shift: 10, e: 17 } },
    skills: {
      primary: {
        ratio: 0.75,
...
/** Stat offensive utilisée pour les ratios de dégâts (DEF pour le guerrier). */
export function getClassPowerStat(classId: ClassId): number {
  if (classId === 'warrior') return getWarriorDefPower();
...
export function getClassPowerStatKey(classId: ClassId): ScaleStat {
  if (classId === 'warrior') return 'def';
  return 'atk';
}

/** Dégâts de base d'un sort selon ses lignes de scaling (ATK, DEF, PV max…). */
export function calcSkillBaseDamage(classId: ClassId, skillKey: SkillKey): number {
  ...
  for (const line of lines) {
    ...
    else if (line.stat === 'def') total += getWarriorDefPower() * line.mult;
...
export function formatDestinySkillDetailHtml(classId: ClassId, skillKey: AbilityKey): string {
  ...
  const chips = formatSkillDamageScalingHtml(classId, skillKey);
  ...
  if (def.ratio > 0 && damageLines.length === 0) {
    const pct = Math.round(def.ratio * 100);
    const statLabel = SCALE_STAT_LABEL[getClassPowerStatKey(classId)];
    meta += `<div class="destiny-skill-meta-row">
      <span class="destiny-skill-meta-label">Ratio</span>
      <span class="destiny-skill-meta-value destiny-skill-meta-value--atk">${pct}% ${statLabel}</span>
    </div>`;
  }
