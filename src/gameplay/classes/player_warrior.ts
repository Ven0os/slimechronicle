...
export class Warrior extends PlayerBase {
    // ... (Code de base inchangé jusqu'à useSkill) ...
    constructor() {
        super('warrior');
    ...
    updateClassPassives(dt) {
        ...
        if (resourceEl) {
            if (this.storedParryDamage > 0) {
                resourceEl.innerHTML = `<div style="color:#9b59b6; font-weight:bold; text-shadow:0 0 5px #4a235a;">🛡 CHARGE: ${Math.floor(this.storedParryDamage)}</div>`;
    ...
    takeDamage(amount) {
        ...
        if (isParrying) {
            const mitigated = amount * 0.75;
            this.storedParryDamage = (this.storedParryDamage || 0) + mitigated;
            amount *= 0.25;
    ...
    updateBuffs(dt) {
        ...
        if(!parryBuff && this.storedParryDamage > 0) {
            const explodeDmg = getWarriorDefPower() * 1.6 + (this.storedParryDamage * 0.5); 
        ...
        setTimeout(() => {
            ...
            Globals.enemies.forEach(e => {
                if(e.position.distanceTo(this.position) < 4.5) { 
                    ...
                    if(dir.dot(toE) > 0.4) { 
                        const dmg = getWarriorDefPower() * 1.3;
        ...
        if(key === 'space') { 
            ...
            const jumpAnim = () => {
                ...
                if(elapsed >= jumpDur) {
                    ...
                    createSkillVisual('shockwave', this.position, 15, 0x8e44ad); createDamageText("CRUSH!", this.position, '#ffffff');
                    ...
                    Globals.enemies.forEach(e => { if(e.position.distanceTo(this.position) <= 15) { e.takeDamage(getWarriorDefPower() * 3.0); e.pushBack(this.position, 20); } });
                    return;
                }
                this.speed = 0;
            ...
            createDamageText("RAGE!", this.position, '#fff'); this.heal(getWarriorDefPower() * 1.5);
