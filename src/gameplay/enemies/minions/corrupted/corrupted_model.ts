...
const CORRUPT_HP_BAR = 0xd946ef;
const CORRUPT_BODY = 0x3b0764;
const CORRUPT_ACCENT = 0xc026d3;
...
export class CorruptedModel {
    ...
    init() {
        ...
        const matBody = new THREE.MeshStandardMaterial({ color: CORRUPT_BODY, roughness: 0.75, flatShading: true });
        const matSpike = new THREE.MeshStandardMaterial({
            color: CORRUPT_ACCENT, emissive: 0x6b21a8, emissiveIntensity: 0.25, roughness: 0.5, flatShading: true,
        });
        const matCore = new THREE.MeshStandardMaterial({ color: 0x12061f, roughness: 0.9 });

        this.parts.torso = new THREE.Group();
        this.parts.torso.position.y = 0.8;
        this.enemy.mesh.add(this.parts.torso);

        const chest = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.22, 0.58, 6), matBody);
        this.parts.torso.add(chest);

        const spikeL = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.35, 4), matSpike);
        spikeL.position.set(-0.34, 0.18, 0);
        spikeL.rotation.z = 0.5;
        this.parts.torso.add(spikeL);

        const spikeR = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.35, 4), matSpike);
        spikeR.position.set(0.34, 0.18, 0);
        spikeR.rotation.z = -0.5;
        this.parts.torso.add(spikeR);

        const core = new THREE.Mesh(new THREE.OctahedronGeometry(0.16, 0), matSpike);
        core.position.set(0, 0.04, 0.2);
        ...
        const claw = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.38, 4), matSpike);
        claw.rotation.x = Math.PI / 2;
        claw.position.set(0, -0.28, 0.14);
    ...
    updateAnim(dt) {
        ...
        if (this.parts.core) this.parts.core.rotation.y += dt * 1.2;
